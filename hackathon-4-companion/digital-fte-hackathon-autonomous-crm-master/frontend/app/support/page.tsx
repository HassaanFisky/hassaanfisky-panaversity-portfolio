...
export default function SupportPage() {
  ...
  const inputStyles =
    "w-full bg-transparent border-b-2 border-border-fine px-0 py-3 text-lg font-serif text-text-primary placeholder-text-muted focus:outline-none focus:border-accent transition-colors rounded-none";
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      ...
      default: return 'bg-bg-elevated text-text-muted border-border-fine';
    }
  };
  ...
  return (
    <main className="min-h-screen bg-bg-base text-text-primary flex flex-col items-center">
      <Navbar />
      ...
      <div className="w-full max-w-[700px] px-6 pt-40 pb-24 flex-grow flex flex-col">
        {!showResults && (
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-sm font-medium text-text-muted hover:text-text-primary transition-colors mb-12 w-fit"
          >
            ...
            {[1, 2, 3].map((s) => (
              <div
                ...
                className={`h-1 flex-1 rounded-full transition-all duration-500 ${
                  s <= step ? "bg-accent" : "bg-border-fine"
                }`}
              />
            ))}
          </div>
          ...
          {showResults ? (
            ...
            >
              <div className="flex flex-col items-center text-center mb-8">
                <div className="w-16 h-16 bg-emerald-50 dark:bg-emerald-900/20 rounded-full flex items-center justify-center mb-6 shadow-sm border border-emerald-100 dark:border-emerald-800">
                  <CheckCircle2 className="w-8 h-8 text-emerald-600 dark:text-emerald-400" />
                </div>
                <h1 className="text-4xl font-serif mb-2">ARIA has processed your request</h1>
                <p className="text-text-muted">Intelligence layer results below</p>
              </div>

              {/* Status Badges */}
              <div className="flex flex-wrap gap-3 justify-center">
                <div className={`px-4 py-1.5 rounded-full text-[10px] font-bold border uppercase tracking-widest ${getPriorityColor(classification.priority)}`}>
                  {classification.priority} Priority
                </div>
                <div className="px-4 py-1.5 rounded-full text-[10px] font-bold border border-border-fine bg-bg-surface text-text-secondary uppercase tracking-widest">
                  {classification.category}
                </div>
                <div className="px-4 py-1.5 rounded-full text-[10px] font-bold border border-border-fine bg-bg-surface text-text-secondary uppercase tracking-widest capitalize">
              ...
              {classification.urgencyScore >= 8 && (
                <div className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-xl p-4 flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-red-800 dark:text-red-400">Your case has been flagged for priority review.</span>
                </div>
              )}
              ...
              <div className="document-card p-8 md:p-10 relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 mb-6">
                   <div className="w-8 h-8 rounded-full bg-text-primary flex items-center justify-center text-white text-[10px] font-bold">ARIA</div>
                   <span className="text-[10px] font-bold tracking-widest text-text-primary uppercase">OFFICIAL RESPONSE</span>
                </div>
                <div className="text-xl font-serif leading-relaxed text-text-primary whitespace-pre-wrap italic">
                  &quot;{supportResponse}&quot;
                </div>
                <div className="mt-8 pt-6 border-t border-border-fine flex items-center justify-between">
              ...
              <div className="flex justify-center pt-8">
                <button 
                  onClick={handleReset}
                  className="inline-flex items-center gap-2 text-[10px] font-bold text-accent hover:text-text-primary transition-colors uppercase tracking-widest"
                >
