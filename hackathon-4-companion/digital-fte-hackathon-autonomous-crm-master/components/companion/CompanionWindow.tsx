...
/**
 * CompanionWindow — Two-pane interior of the Companion glass panel.
 *
 * Layout:
 *   ┌──────────────────────────────────────────────────────┐
 *   │  CompanionSidebar (240px, collapsible)  │  CompanionChat (flex-1)  │
 *   └──────────────────────────────────────────────────────┘
 *
 * Desktop: sidebar collapses inline (spring width 0 → 240px).
 * Mobile:  sidebar overlays as a drawer; drag="x" gesture dismisses it.
 *          Framer Motion drag handles swipe-to-close naturally.
 *
 * RTL (language === "ur"): sidebar renders on the right side.
 */
...
export function CompanionWindow({
  ...
  // ── Sidebar motion values — RTL-aware ─────────────────────────────────────
  // Desktop: width collapses (0 ↔ 240).
  // Mobile:  slides in/out as translateX overlay.
  ...
  return (
    <div
      ...
      dir={isRTL ? "rtl" : "ltr"}
    >
      {/* ── Desktop sidebar: inline width-collapse ────────────────────────── */}
      ...
      {/* ── Mobile sidebar: overlay drawer with swipe-to-dismiss ─────────── */}
      <AnimatePresence initial={false}>
        {sidebarOpen && (
          <>
            {/* Scrim */}
            ...
            <motion.aside
              ...
              className={`
                ...
                w-60 z-[2] bg-bg-base shadow-xl border-r border-border-fine
                overflow-hidden
              `}
