...
export function SnowOverlay() {
  ...
  useEffect(() => {
    ...
    // Sync with ActionDock snow toggle
  ...
  // Determine if snow should run (either explicit button toggle or dark mode)
  // According to user's voice note: "dark mode karte hi snow ho aur lagay ke jese snow start hogai... glassy aur lightning effect"
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < flakeCount; i++) {
        ...
        flakes.push({
            ...
            size: depth * 2.5 + 0.5,
            v: depth * 2.0 + 0.5, // Natural falling speed
            swing: Math.random() * Math.PI * 2, // For horizontal swaying
            swingSpeed: Math.random() * 0.015 + 0.005,
            opacity: depth * 0.7 + 0.3 // Closer = more opaque
        });
    }
