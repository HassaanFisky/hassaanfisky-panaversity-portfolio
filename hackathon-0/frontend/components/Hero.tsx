...
export function Hero() {
  const completedPoints = hackathons
    ...
    .reduce((acc, h) => acc + parseInt(h.points.replace(/,/g, "").split(" ")[0]), 0);
  ...
  return (
    <section className="weather-hero min-h-[95vh] flex flex-col items-center justify-center pt-32 pb-24 md:pb-32 overflow-hidden bg-transparent transition-colors duration-500">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        ...
        />
        <div className="absolute top-[20%] left-[10%] w-[400px] h-[400px] bg-accent/10 rounded-full blur-[120px] animate-pulse" style={{ willChange: "opacity" }} />
      ...
      >
        <MotionDiv variants={fadeUp} className="space-y-10">
          ...
          <div className="inline-flex items-center gap-3 px-6 py-2.5 rounded-full glass-apple border border-accent/30 text-[10px] font-black uppercase tracking-[0.5em] text-accent mb-6 shadow-float">
            ...
            <span className="relative flex h-2 w-2 shrink-0">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" style={{ willChange: "opacity, transform" }} />
          ...
          <p className="prose-editorial text-xl md:text-3xl max-w-4xl mx-auto leading-tight text-text-secondary/80 font-serif italic mb-16 px-4">
            Building high-fidelity <span className="text-text-primary font-black not-italic border-b-2 border-accent/40">AI Systems</span> &mdash;
        ...
        <MotionDiv variants={fadeUp} className="flex flex-col xl:flex-row items-center justify-center gap-12">
          ...
          <div className="flex items-center gap-10 sm:gap-12 px-8 sm:px-10 py-5 sm:py-6 glass-apple border-white/20 rounded-[2rem] shadow-float shrink-0">
            <div className="flex items-center gap-4 sm:gap-5 group">
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-accent/10 border border-accent/30 flex items-center justify-center text-accent shadow-sm transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-12" style={{ willChange: "transform" }}>
                <Trophy size={24} strokeWidth={2.5} />
              </div>
              <div className="text-left">
                <div className="text-3xl sm:text-4xl font-serif text-text-primary font-black leading-none tracking-tighter">{completedPoints.toLocaleString()}</div>
                <div className="text-[9px] uppercase tracking-[0.4em] font-black text-text-muted mt-2">Total Points</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 sm:gap-5 group">
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-accent/5 border border-accent/10 flex items-center justify-center text-accent/70 shadow-sm transition-transform duration-300 group-hover:scale-110 group-hover:rotate-12" style={{ willChange: "transform" }}>
                <Layers size={24} strokeWidth={2.5} />
              </div>
              <div className="text-left">
                <div className="text-3xl sm:text-4xl font-serif text-text-primary font-black leading-none tracking-tighter">5/5</div>
                <div className="text-[9px] uppercase tracking-[0.4em] font-black text-text-muted mt-2">Projects Live</div>
              </div>
            </div>
          </div>
        </MotionDiv>
        ...
        >
          <div
            ...
            onClick={() => document.getElementById("hackathon-grid")?.scrollIntoView({ behavior: "smooth" })}
          >
            <span className="text-[8px] tracking-[0.6em] uppercase font-black text-text-muted">Scroll to explore</span>
            <div className="w-[2px] h-20 bg-gradient-to-b from-accent to-transparent animate-bounce" style={{ willChange: "transform" }} />
          </div>
        </MotionDiv>
      </MotionDiv>
    </section>
