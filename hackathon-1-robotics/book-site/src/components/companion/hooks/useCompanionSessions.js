/**
 * useCompanionSessions — localStorage-backed session CRUD (H1 Docusaurus variant).
...
const GREETINGS = {
  ...
  H1: "Hello! I am Aira, your Physical AI Textbook companion. Ask me anything about robotics.",
...
const storageKey = (platform) => `aira-sessions-${platform}`;
...
function createSession(platform) {
  ...
  return {
    ...
    messages: [{
      role:      "assistant",
      content:   GREETINGS[platform] || "Hello! How can I help you?",
      timestamp: now,
    }],
  };
}
...
export function useCompanionSessions(platform) {
  ...
  const updateActiveMessages = useCallback((messages) => {
